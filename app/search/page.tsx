...
// --- データ取得 ---
async function getFilteredArticles(tag?: string, categoryId?: string) {
  let filters = "";

  if (tag) {
    filters = `problem_tags[contains]${tag}[or]solution_tags[contains]${tag}`;
  } else if (categoryId) {
...
export default async function SearchPage({
  ...
  const articles = await getFilteredArticles(tag, categoryId);

  let pageTitle = "記事一覧";
  if (tag) pageTitle = `#${tag}`;
  if (categoryName) pageTitle = categoryName || "カテゴリ検索";

  const breadcrumbTitle = tag ? `タグ: ${tag}` : (categoryName || "検索結果");
    ...
    <main className="bg-[#F9FAFB] min-h-screen pt-14 md:pt-16 pb-20">
      ...
      <div className="bg-white border-b border-gray-100 relative overflow-hidden">
          <div className="container mx-auto px-4 md:px-6 max-w-6xl py-4 md:py-6 relative z-10">
             <nav className="flex items-center text-[10px] md:text-xs text-gray-400 font-en gap-2">
                ...
                <span className="text-melon-dark font-bold truncate max-w-[200px]">{breadcrumbTitle}</span>
            </nav>
          </div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl py-12">
        ...
        <div className="text-center mb-12">
            <span className="text-melon-dark font-bold tracking-widest font-en text-xs uppercase mb-3 block">SEARCH RESULT</span>
      ...
      <StickyBottomNav title={breadcrumbTitle} />
